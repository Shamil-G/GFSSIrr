...
def get_select():
    return """
		select
			...
			e.event_date as "Дата проведения мероприятия",
			e.rfbn_id as  "Код области",
			b.name as "Область",
			e.participants as "Участники мероприятия (организация)",
			e.description as "Сведения о мероприятии",
			e.refer as "Размещение в соц.сетях (ссылка)",
			employee as "Исполнитель"
		from round_table_events e, loader.branch b
...
report_name = 'Сведения об участии в работе брифингов, круглых столов, конференций, заседаний комиссий и т.п.'
report_code = 'КС_01'
...
def report_01(params, filename=f"rep_{report_code}.xlsx"):
	...
	with xlsxwriter.Workbook(output, {'in_memory': True}) as workbook:
		...
		##
		list_name_number = []
		list_name_date = ['Дата проведения мероприятия']
		list_name_text = ["Область","Участники мероприятия (организация)", "Сведения о мероприятии", "Размещение в соц.сетях (ссылка)", "Исполнитель"]
		list_name_text_list = []
		list_name_text_center = ["Код области", "Номер протокола"]

		list_column_width =[11, 12, 14, 10, 40, 40, 40, 35, 35]
