...
def get_select():
    return """
		select
			...
			e.event_date as "Дата публикации",
			e.rfbn_id as  "Код области",
			b.name as "Область",
			e.smi_name as "Наименование СМИ",
			e.description as "Наименование статьи, номер или ссылка",
			employee as "Исполнитель"
		from smi_events e, loader.branch b
...
report_name = 'Сведения о размещении информации в региональных печатных и электронных средствах массовой информации (СМИ)'
report_code = 'СМ_01'
...
def report_01(params, filename=f"rep_{report_code}.xlsx"):
	...
	with xlsxwriter.Workbook(output, {'in_memory': True}) as workbook:
		...
		##
		list_name_number = []
		list_name_date = ['Дата публикации']
		list_name_text = ["Область","Наименование СМИ", "Наименование статьи, номер или ссылка", "Исполнитель"]
		list_name_text_list = []
		list_name_text_center = ["Код области", "Номер протокола"]

		list_column_width =[11, 12, 14, 10, 40, 40, 45, 35]
