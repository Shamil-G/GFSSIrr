...
export const EditRowTableBinder = {
    ...
    attach(row) {
        ...
        row.addEventListener('click', (e) => {
            ...
            console.log("cancel edit-row. year: ", year);
    ...
    startEdit(id, year, field) {
        const row = document.querySelector(`tr[data-role="edit-row"][data-id="${id}"][data-year="${year}"]`);
        if (!row) {
            console.warn(`❌ startEdit: строка с id="${id}" и year="${year}" не найдена`);
            return;
        }
    ...
    cancelEdit(id, year, field) {
        //console.log('EditField. cancelEdit. id:', id, 'field:', field);
        const row = document.querySelector(`tr[data-role="edit-row"][data-id="${id}"][data-year="${year}"]`);
        if (!row) {
            console.warn(`❌ cancelEdit: строка с id="${id}" и year="${year}" не найдена`);
            return;
        }
        const input = row?.querySelector(`input[name="${field}"]`);
        if (!input) {
    ...
    async save(id, year, field) {
        const context = PageManager?.get();

        const row = document.querySelector(`tr[data-role="edit-row"][data-id="${id}"][data-year="${year}"]`);
        if (!row) {
            console.warn(`❌ save: строка с id="${id}" и year="${year}" не найдена`);
            return;
        }
        ...
        const req = context.getRequest('big_ref._context', 'save_row_big_ref', id, year, field, value);
