...
export const FilterInputBinder = {
    role: 'filter-input',
    massive: true,

    attach(el) {
        if(!el) return;
        if (el.__filter_input) return;
        el.__filter_input = true;
        ...
        if (tag === 'INPUT') {
            ...
            if (!url || !targetId) {
                console.warn('❌ FilterInputBinder: missing url or targetId on INPUT', el);
                return;
            }
        ...
        if (isInteractive) {
            ...
            if (!url || !targetId || !input) {
                console.warn('❌ FilterInputBinder: missing url, targetId, or input', el);
                return;
            }
            ...
            el.addEventListener('click', (event) => {
                ...
                console.log('filter-input. addEventListener to ', input, "value: ", value);
                TableLoader.load(url, targetId, { value });
            });
        }
    },
    ...
    attachAll(zone = document) {
        ...
        triggers.forEach(el => {
/*            console.log(`FilterInputBinder.attachAll: tag = ${el.tagName}`);*/
            this.attach(el);
        });
    }
