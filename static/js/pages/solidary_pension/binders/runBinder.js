...
export const RunBinder = {
    ...
    resolveFragment(context, fragment_url, targetId) {
        ...
        if (!url) {
            const req = context.getRequest("base_pension._context", "fragment");
            url = req?.url;
        }

        let target = targetId?.trim() || null;
        if (!target) {
            const entry = context.tabContext.getEntry("base_pension._context");
    ...
    attach(dropdown, handler = null, force = false) {
        ...
        console.log("RUN-BINDER. Нашли dropdown: ", dropdown, 'button: ', button);
        ...
        console.log('RUN-BINDER. Target:', targetId, ', FRAGMENT_URL:', fragment_url, ', action_url: ', action_url, ', taskName:', taskName, 'work_url: ', workUrl);
        ...
        button.addEventListener('click', (event) => {
                ...
                if (action_url) {
                    try {
                        const body = { taskName: taskName, scenario: sessionScenario, work_url: workUrl }
                        console.log('*** RUN-BINDER. BODY: ', body);
                        fetch(action_url, {
    ...
    attachAll(handler = null, zone = document) {
        ...
        dropdowns.forEach(dropdown => {
            ...
            if (!['DIV','SECTION'].includes(tag)) {
                console.warn(`⚠️ RUN-BINDER: skipping non-DIV element <${tag}>`, dropdown);
                return;
            }
        ...
        if (!this.__socketBound) {
            ...
            socket.on("task_finished", (data) => {
                ...
                console.log('RUN-BINDER. TASK FINISHED. DATA: ', data, ', task_id: ', task_id, ', task_name: ', taskName, ', result: ', result)
                ...
                console.log('RUN-BINDER. Для обновления фрагмента используем. URL: ', url, ', targetId: ', targetId, ', Result: ', result);
                if (targetId && url) {
                    console.log('RUN-BINDER. Обновляем данные. URL: ', url, ', targetId: ', targetId, ', Result: ', result);
